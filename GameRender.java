...
public class GameRender implements MouseMotionListener, MouseListener, MouseWheelListener, KeyListener {
	...
	private void networkTransmit() throws IOException{
		...
		gameState = recieveDataHandler.gameState;
	...
	private void renderThings(Graphics2D g){
		...
		switch(gameState)
		{
		case GameState.GAME_BLUE_GOAL_SCORED:
			text.add("BLUE");
			text.add("GOAL");
			text.add("SCORED");
			splashColor = Color.BLUE;

			splash = generateSplashScreen(splashColor,(float) .375, Color.BLACK,text);
			g.drawImage(splash,screenWidth/2-splash.getWidth()/2, screenHeight/2-splash.getHeight()/2, null);

			break;
		case GameState.GAME_RED_GOAL_SCORED:
			text.add("RED");
			text.add("GOAL");
			text.add("SCORED");
			...
			break;
		case GameState.GAME_PERIOD_OVER:
			text.add("PERIOD "+gamePeriod);
			text.add("OVER");
